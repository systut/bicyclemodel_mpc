...
% Robot inputs
    ...
    syms L delta_t
    parameters = [L, delta_t]; % wheel to center, sample time
...
% ----- Export matlab functions -----
    matlabFunction(f,  'File','BM_functions/f_num'  ,'Vars',{x,u,parameters});
    matlabFunction(f_d,'File','BM_functions/f_d_num','Vars',{x_k,u_k,parameters});
    matlabFunction(A_d,'File','BM_functions/A_d_num','Vars',{x_lin,u_lin,parameters});
    matlabFunction(B_d,'File','BM_functions/B_d_num','Vars',{x_lin,u_lin,parameters});
...
% ----- simulate nonlinear & linear discrete time system -----
    for k = 1:length(t_ref)-1
        ...
        A = A_d_num(x_lin,u_lin,param);
        B = B_d_num(x_lin,u_lin,param);
